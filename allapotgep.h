...
class State{
    ...
    bool acceptable;
public:
    void setStateName(char name[21]){strcpy(stateName, name);}
    //H -> false 
    //I -> true
    void setType(char type){
        switch(type){
        case 'I': acceptable = true;
        break;
        case 'H': acceptable = false;
        break;
        }
    }    

    const char* getName(){return stateName;}
    const bool getType(){return acceptable;}
};
...
class Allapotgep {
    ...
    void getStates(std::ifstream &file);
    void freeStates();
    void freeController();
public:
    /**
